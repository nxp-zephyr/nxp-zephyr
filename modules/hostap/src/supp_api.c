...
static int wpas_config_process_blob(struct wpa_config *config, char *name,
	...
	if (params->security != WIFI_SECURITY_TYPE_NONE) {
		...
		if (params->security == WIFI_SECURITY_TYPE_SAE) {
			...
			if (!wpa_cli_cmd_v("set_network %d group %s",
					   resp.network_id, group_ciphers))
				goto out;

			if (!wpa_cli_cmd_v("set_network %d pairwise %s",
					   resp.network_id, pairwise_cipher))
				goto out;

			if (!wpa_cli_cmd_v("set_network %d group_mgmt %s",
					   resp.network_id, group_mgmt_cipher))
				goto out;
			...
			if (phase2 != NULL) {
				if (!wpa_cli_cmd_v("set_network %d phase2 \"%s\"",
						   resp.network_id, phase2))
					goto out;
			}
			...
			if (wpas_config_process_blob(wpa_s->conf, "ca_cert",
						     enterprise_file.ca_cert,
						     enterprise_file.ca_cert_len)) {
				goto out;
			}
			...
			if (enterprise_file.ca_cert2_len) {
				if (wpas_config_process_blob(wpa_s->conf, "ca_cert2",
							     enterprise_file.ca_cert2,
							     enterprise_file.ca_cert2_len))
					goto out;
				...
				if (!wpa_cli_cmd_v("set_network %d ca_cert2 \"blob://ca_cert\"",
						   resp.network_id))
					goto out;
			}
			...
			if (enterprise_file.client_cert2_len) {
				if (wpas_config_process_blob(wpa_s->conf, "client_cert2",
							     enterprise_file.client_cert2,
							     enterprise_file.client_cert2_len))
					goto out;

				if (!wpa_cli_cmd_v("set_network %d client_cert2 \""
						   "blob://client_cert2\"", resp.network_id))
					goto out;
			} else {
				if (!wpa_cli_cmd_v("set_network %d client_cert2 \""
						   "blob://client_cert\"", resp.network_id))
					goto out;
			}

			if (enterprise_file.client_key2_len) {
				if (wpas_config_process_blob(wpa_s->conf, "private_key2",
							     enterprise_file.client_key2,
							     enterprise_file.client_key2_len))
					goto out;

				if (!wpa_cli_cmd_v("set_network %d private_key2 \""
						   "blob://private_key2\"", resp.network_id))
					goto out;
				...
				if (!wpa_cli_cmd_v("set_network %d private_key2 \""
						   "blob://private_key\"", resp.network_id))
					goto out;
...
void hapd_update_channel(uint8_t *channel)
{
	...
	if (wpa_s->wpa_state == WPA_COMPLETED) {
		...
		*channel = con_chan;
		wpa_printf(MSG_DEBUG, "STA has connection on channel %d, "
			   "setup AP on same channel", *channel);
	}
}
