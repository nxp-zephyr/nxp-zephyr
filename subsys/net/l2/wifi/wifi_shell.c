				 ...
				 enum wifi_iface_mode iface_mode)
{
	...
	while ((opt = getopt_long(argc, argv, "s:p:k:e:w:b:c:m:t:a:K:S:T:V:I:P:R:d:h",
		...
		case 'V':
			...
			break;
		case 'I':
			if (params->nusers >= WIFI_IDENTITY_MAX_USERS) {
				PR_WARNING("too many eap identities (max %d identities)\n",
					    WIFI_IDENTITY_MAX_USERS);
				return -EINVAL;
			}
			...
			break;
		case 'P':
			if (params->passwds >= WIFI_IDENTITY_MAX_USERS) {
			...
			params->passwds++;
			if (params->eap_passwd_length > WIFI_PSWD_MAX_LEN) {
...
static int cmd_wifi_wps_pin(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (net_mgmt(NET_REQUEST_WIFI_WPS_PIN, iface, &params, sizeof(params))) {
		PR_WARNING("Start wps pbc connection failed\n");
		return -ENOEXEC;
	}
...
SHELL_STATIC_SUBCMD_SET_CREATE(wifi_cmd_ap,
	...
	SHELL_CMD_ARG(enable, NULL,
		  ...
		  "7: WPA-Auto-Personal, 8: DPP, 9: EAP-TLS, 10: EAP-PEAP-MSCHAPv2\n"
		  "11: EAP-PEAP-GTC, 12: EAP-TTLS-MSCHAPv2, 13: EAP-PEAP-TLS\n"
...
SHELL_STATIC_SUBCMD_SET_CREATE(
	...
	SHELL_CMD_ARG(wps_pin, NULL,
		      "Set and get wps pin.\n"
		      "[pin] No need input pin for get operation, for set operation is pin value.\n",
